...
class WSv2 extends EventEmitter {
   ...
   * The last-seen sequence # is updated internally
   *
   * @param {Array} msg
  ...
  _validateMessageSeq (msg = []) {
    ...
    // only used for chan 0; heartbeats use the public sequence
    ...
    if (this._lastPubSeq === -1) { // init pub seq
      this._lastPubSeq = seq
      return null
    }

    if (seq !== this._lastPubSeq + 1) { // audit
  ...
  onTicker ({ symbol, cbGID }, cb) {
    this._registerListener('ticker', { 0: symbol }, Ticker, cbGID, cb)
  }
