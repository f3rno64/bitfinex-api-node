...
class RESTv2 {
   ...
   * @private
   */
  _makeAuthRequest (path, payload = {}, cb = () => {}, modelClass) {
    ...
    return rp({
      ...
      this._doTransform.bind(this, modelClass)
    ).then(res => {
      cb(null, res)
   ...
   * @private
   */
  _makePublicRequest (path, cb = () => {}, modelClass) {
    const url = `${this._url}/v2${path}`
  ...
  ticker (symbol = 'tBTCUSD', cb) {
    return this._makePublicRequest(`/ticker/${symbol}`, cb, Ticker)
  }
  ...
  tickers (symbols = [], cb = () => {}) {
    ...
    return this._makePublicRequest(`/tickers?symbols=${symbols.join(',')}`, cb, Ticker)
  }
